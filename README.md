...
- **Secure Cross-Device Sync (Zero-Knowledge)**: Allows developers to securely sync secrets between multiple machines. The sync file is encrypted on-device using AES-256-GCM with keys derived via memory-hard **Argon2id** (Time=3, Memory=64MB, Parallelism=4) from a user passphrase. Plaintext credentials never touch network layers or cloud storage.
